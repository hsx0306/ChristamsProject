...
mongoose.connect('mongodb+srv://1317hsb:21Mbt8ppjFjhKOpk@christmas-user.dpy6wtl.mongodb.net/UserData');
...
app.use('/LoginRegister', express.static(path.join(__dirname, 'Login-Register')));
...
app.get('/UserData/:id', (req, res) => {
  app.use('/UserData', express.static(path.join(__dirname, `UserData/${req.params.id}.json`)));
  ...
  if(fs.existsSync(`./UserData/${req.params.id}.json`)) {
    // JSON 파일을 읽어서 파싱
    const jsonData = JSON.parse(fs.readFileSync(`./UserData/${req.params.id}.json`, 'utf-8'));

    // 필요한 데이터만 추출
    const extractedData = {};
    extractedData.santaName = jsonData[`${req.params.id}`].santaName;
    extractedData.boxes = Object.values(jsonData[`${req.params.id}`]).filter(value => typeof value === 'object').map(box => ({
...
app.post('/OauthVerification', function(req, res){
  ...
  if((emailRegex.test(req.body.id) === false) || (passwordRegex.test(req.body.password) == false)) {
    ...
    UserData.findOne({ id: `${req.body.id}` })
      .then(result => {
        ...
        if(!result) {
          ...
          const salt = '@l9_hsx';
          const hashPassword = crypto.createHash('sha512').update(req.body.password + salt).digest('base64');
          ...
          if(hashPassword == result.password){
            ...
            return res.status(200).send({token: `${token}`, ls: `${result.id}`});
          }
  
  
          else { console.log("로그인에러"); return res.status(400).send('Loginerror'); }
